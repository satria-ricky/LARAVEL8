...
<body id="body" data-spy="scroll" data-target=".navbar" data-offset="100">
    ...
    <div class="content-wrapper" style="margin-top: 130px;">
        <div class="container">
            <section class="contact-us mb-5" id="features-section">
                ...
                <div class="content">
                <label for="labelJenis">Jenis Bengkel</label>
                 <select class="form-control" id="labelJenis" onchange="getPasarByjenis()">
                    @foreach ($jenisBengkel as $item)
                        <option value="{{ $item->id_jenis_bengkel }}"> {{ $item->nama_jenis}} </option>
                    @endforeach
                 </select>
                </div>
    ...
    <script>
        ...
        function getPasarByjenis() {
            var id = document.getElementById("labelJenis").value;
            $.ajax({
                type: 'post',
                url: "{{ url('getPasarByjenis') }}",
                data: {
                    _token: '{{ csrf_token() }}',
                    id_jenis: id
                },
                            ...
                            if (data[i].latitude != null || data[i].longitude != null) {
                                ...
                                marker.bindPopup(`
                                  <div class="card" style="width: 15rem; height:15rem;">
                                    <div class="card-body">
                                      <h3 class="card-title">` + data[i].nama_bengkel + `</h3>
                                      <p class="card-text">` + data[i].alamat + `.</p>
                                      <div class="btn-group">
                                        <form action="detilBengkel" method="post">
                                            @csrf
                                            <input type="hidden" name="id" value="` + data[i].id_bengkel + `">
